...
export const TableNode = memo(function TableNode({ data }: TableNodeProps) {
  ...
  return (
    <div className="bg-gray-900 rounded-lg border border-gray-700 shadow-lg overflow-hidden min-w-[280px]">
      <div
        className="px-3 py-2.5 text-sm font-semibold text-white flex items-center gap-2"
      ...
      <div className="divide-y divide-gray-800">
        {table.columns.map((col) => {
          ...
          return (
            <div
              key={col.name}
              className={`flex items-center justify-between px-3 py-2 text-xs border-l-2 ${borderColor} hover:bg-gray-800/50 group relative`}
            >
              <Handle
                type="target"
              ...
              <div className="flex items-center gap-2">
                {col.isPrimaryKey && <span className="text-yellow-400 text-[10px]">PK</span>}
                {col.isForeignKey && <span className="text-blue-400 text-[10px]">FK</span>}
                <span className="text-gray-200">{col.name}</span>
              </div>
              <span className={`text-gray-500 ${col.isNullable ? 'italic' : ''}`}>
                {col.type}
              </span>
              <Handle
                type="source"
